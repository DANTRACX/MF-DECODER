...
void subroutine_08_MAIN()
{
    ...
    do
    {
        ...
        if(DECODER.PROTOCOL_FETCH(&currentCmd) == 1)
        {
            if(currentCmd != prevCmd)
            {
                ...
                else if(currentCmd == SIGNAL_ADDR[4])
                {
                    ...
                    {
                        if((HAUPTSIGNAL.STATUS != HP00_active) && (HAUPTSIGNAL.STATUS != HP00_pending) && (HAUPTSIGNAL.STATUS != SH1_active) && (HAUPTSIGNAL.STATUS != SH1_pending))
                        {
                            VORSIGNAL_CLEAR(&VORSIGNAL);
                            SCHEDULER_SECONDS_SET_TIME(&VORSIGNAL.DELAY);
                        }
        ...
        /* VORSIGNAL -> DELAY-TO-VR0/VR1/VR2 */
